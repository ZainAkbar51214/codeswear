...
const Navbar = () => {
    const toggleCart = () => {
        if(ref.current.classList.includes('translate-x-full')){
            ref.current.classList.remove('translate-x-full')
            ref.current.classList.add('translate-x-0')
        }else if(!ref.current.classList.includes('translate-x-full')){
    ...
    return (
        <>
            <nav className={`${globalNavbar.navContainer}`}>
                ...
                <div ref={ref} className="sideCart absolute top-0 right-0 bg-slate-400 p-10 transition-transform transform translate-x-full">
